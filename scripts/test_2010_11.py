...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2010-11.html');page.wait_for_url('**/2010-11.html');page.wait_for_selector('.bar-row')
    ...
    assert '2010-11' in dl.value.suggested_filename
    rows=list(csv.DictReader(io.StringIO(Path(dl.value.path()).read_text(encoding='utf-8-sig'))))
    assert rows and all(r['Period']=='2010\u201311' for r in rows)
    ...
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2010-11-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2010-11-mobile.png'),full_page=True)

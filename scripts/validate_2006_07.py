...
data=json.loads((ROOT/'data/dashboard-2006-07.json').read_text(encoding='utf-8'))
assert len(data['tables'])==33
books={};count=0
for t in data['tables']:
    ...
    for row in t['rows']:
        ...
        for h,c in zip(t['headers'],row['cells']):
            r,col=row['row']-1,h['column'];raw=s.cell_value(r,col)
...
assert 'debt finance' in by['3-1']['headers'][3]['denominator']
assert 'equity finance' in by['3-1']['headers'][6]['denominator']
assert by['4-2']['headers'][0]['measure']==by['4-2']['headers'][8]['measure']
assert len([r for r in by['4-1']['rows'] if r['group']=='Industry'])==16
assert all(r['group']!='Overall' for r in by['4-2']['rows'])
assert len([r for r in by['4-2']['rows'] if r['label']=='Total'])==17
flags=Counter(c['status'] for t in data['tables'] for r in t['rows'] for c in r['cells'])
assert flags['unreliable']==9
print(f'PASS: {count:,} observations match Excel; numeric coverage, flags, nested totals, header spans and denominators verified')
print(dict(flags))

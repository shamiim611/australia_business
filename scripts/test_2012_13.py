...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2012-13.html');page.wait_for_url('**/2012-13.html');page.wait_for_selector('.bar-row')
    ...
    assert '2012-13' in dl.value.suggested_filename
    rows=list(csv.DictReader(io.StringIO(Path(dl.value.path()).read_text(encoding='utf-8-sig'))))
    assert rows and all(r['Period']=='2012\u201313' for r in rows)
    ...
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2012-13-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2012-13-mobile.png'),full_page=True)
    ...
    browser.close()
    print('PASS: all 38 tables, populations and four modes; denominators, CSV, year navigation, overview, mobile width and no JS errors')

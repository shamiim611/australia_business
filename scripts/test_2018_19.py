...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2018-19.html');page.wait_for_url('**/2018-19.html');page.wait_for_selector('.bar-row')
    assert '?' not in page.locator('body').inner_text()
    assert page.locator('#topics button').count()==11
    assert page.locator('#heatmap tbody tr').count()==17
    assert page.locator('#overviewGroup option').count()==2
    expected=page.evaluate("['9-1','9-1','9-1','1-1'].map((id,i)=>{const t=window.BUSINESS_DATA.tables.find(t=>t.id===id);return t.rows.find(r=>r.group==='Overall').cells[[0,1,4,1][i]].value.toFixed(1)+'%';})")
    assert page.locator('.kpi strong').all_text_contents()==expected
    page.select_option('#overviewGroup','Employment size');assert page.locator('#heatmap tbody tr').count()==5
    page.select_option('#overviewGroup','Industry')
    page.locator('[data-topic="9"]').click();page.select_option('#metric','2')
    ...
    page.locator('[data-topic="11"]').click()
    assert '2017' in page.locator('#source').inner_text()
    ...
    page.locator('[data-topic="5"]').click();page.select_option('#table','5-3')
    assert page.locator('#population').input_value()=='Innovation-active businesses'
    page.select_option('#table','5-4')
    assert page.locator('#population').input_value()=='Non innovation-active businesses'
    page.locator('[data-topic="1"]').click()
    (ROOT/'artifacts').mkdir(exist_ok=True)
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2018-19-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2018-19-mobile.png'),full_page=True)
    ...
    browser.close()
    print('PASS: all 47 explorer tables, populations and four modes; denominators, CSV, year navigation, overview, mobile width and no JS errors')

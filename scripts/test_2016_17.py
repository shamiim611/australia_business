...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2016-17.html');page.wait_for_url('**/2016-17.html');page.wait_for_selector('.bar-row')
    assert '?' not in page.locator('body').inner_text()
    assert page.locator('#topics button').count()==7
    assert page.locator('#heatmap tbody tr').count()==17
    assert page.locator('#overviewGroup option').count()==2
    expected=page.evaluate("['4-1','4-1','4-1','4-1'].map((id,i)=>{const t=window.BUSINESS_DATA.tables.find(t=>t.id===id);return t.rows.find(r=>r.group==='Overall').cells[[0,1,2,4][i]].value.toFixed(1)+'%';})")
    ...
    assert '2016-17' in dl.value.suggested_filename
    rows=list(csv.DictReader(io.StringIO(Path(dl.value.path()).read_text(encoding='utf-8-sig'))))
    assert rows and all(r['Period']=='2016\u201317' for r in rows)
    ...
    page.select_option('#mode','groups')
    page.locator('[data-topic="8"]').click();page.select_option('#table','7-3');page.select_option('#mode','populations')
    assert page.locator('.bar-row').count()==51, 'Three skill populations across 17 industries'
    page.locator('[data-topic="4"]').click()
    (ROOT/'artifacts').mkdir(exist_ok=True)
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2016-17-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2016-17-mobile.png'),full_page=True)
    ...
    browser.close()
    print('PASS: all 36 tables, populations and four modes; denominators, CSV, year navigation, overview, mobile width and no JS errors')

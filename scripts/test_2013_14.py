...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2013-14.html');page.wait_for_url('**/2013-14.html');page.wait_for_selector('.bar-row')
    ...
    expected=page.evaluate("['4-1','4-1','4-1','5-1'].map((id,i)=>{const t=window.BUSINESS_DATA.tables.find(t=>t.id===id);return t.rows.find(r=>r.group==='Overall').cells[[10,11,14,2][i]].value.toFixed(1)+'%';})")
    ...
    page.locator('[data-topic="4"]').click();page.select_option('#metric','12')
    ...
    assert '2013-14' in dl.value.suggested_filename
    rows=list(csv.DictReader(io.StringIO(Path(dl.value.path()).read_text(encoding='utf-8-sig'))))
    assert rows and all(r['Period']=='2013\u201314' for r in rows)
    assert all('equity finance' in r['Notes'] for r in rows)
    page.locator('[data-topic="4"]').click();page.select_option('#table','4-3');page.select_option('#metric','8')
    ...
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2013-14-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2013-14-mobile.png'),full_page=True)

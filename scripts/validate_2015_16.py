...
data=json.loads((ROOT/'data/dashboard-2015-16.json').read_text(encoding='utf-8'))
...
assert 'Purchase' in by['3-4']['headers'][9]['measure']
assert 'Purchase' not in by['3-4']['headers'][10]['measure']
assert 'Australian sources' not in by['2-3']['headers'][4]['label']
assert 'Australian suppliers' not in by['2-6']['headers'][3]['label']
assert len({h['population'] for h in by['4-1']['headers']})==3
assert by['4-3']['headers'][8]['unit']=='$b'
assert len(by['4-3']['rows'])==5
assert 'any organisational' in by['5-5']['headers'][5]['measure']
assert 'any marketing' in by['5-7']['headers'][5]['measure']
assert len(by['8-3']['rows'])==22
assert by['4-1']['headers'][2]['measure']=='social media presence'
assert by['4-1']['headers'][2]['population']=='All businesses'
assert len(by['4-1']['headers'])==15
assert by['7-1']['headers'][3]['measure'].startswith('Lack of skilled')
assert not by['7-1']['headers'][6]['measure'].startswith('Lack of skilled')
assert by['7-2']['headers'][2]['measure'].startswith('Lack of skilled')
assert all(h['population']=='Innovation-active businesses' for h in by['5-9']['headers'])
assert len(by['5-9']['rows'])==18
assert len(by)==47
assert all('sought debt finance' in h['denominator'] for key in ['3-2','3-3'] for h in by[key]['headers'])
flags=Counter(c['status'] for t in data['tables'] for r in t['rows'] for c in r['cells'])
print(f'PASS: {count:,} observations match Excel; source coverage, population pivots, flags, units and header spans verified')
print(dict(flags))

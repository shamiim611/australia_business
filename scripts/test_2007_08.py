...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2007-08.html');page.wait_for_url('**/2007-08.html');page.wait_for_selector('.bar-row')
    ...
    expected=page.evaluate("['4-4','4-4','5-1','1-4'].map((id,i)=>{const t=window.BUSINESS_DATA.tables.find(t=>t.id===id);return t.rows.find(r=>r.group==='Overall').cells[[6,7,2,0][i]].value.toFixed(1)+'%';})")
    ...
    assert '2007-08' in dl.value.suggested_filename
    rows=list(csv.DictReader(io.StringIO(Path(dl.value.path()).read_text(encoding='utf-8-sig'))))
    assert rows and all(r['Period']=='2007\u201308' for r in rows)
    assert all('equity finance' in r['Notes'] for r in rows)
    page.locator('[data-topic="4"]').click();page.select_option('#table','4-4');page.select_option('#metric','8')
    ...
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2007-08-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2007-08-mobile.png'),full_page=True)

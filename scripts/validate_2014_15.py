...
data=json.loads((ROOT/'data/dashboard-2014-15.json').read_text(encoding='utf-8'))
assert len(data['tables'])==35
books={};count=0
...
assert any('Any level of government' in h['measure'] for h in by['2-5']['headers'])
assert all('relying on a small number' in h['denominator'] for h in by['2-7']['headers'][1:])
assert all(('skilled persons' in h['measure'].lower()) == (i % 10 in [3,4,5]) for i,h in enumerate(by['5-1']['headers']))

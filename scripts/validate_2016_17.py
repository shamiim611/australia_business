...
data=json.loads((ROOT/'data/dashboard-2016-17.json').read_text(encoding='utf-8'))
assert len(data['tables'])==36
books={};count=0
...
assert by['6-1']['headers'][3]['measure'].startswith('Lack of skilled')
assert not by['6-1']['headers'][6]['measure'].startswith('Lack of skilled')
assert by['6-2']['headers'][2]['measure'].startswith('Lack of skilled')
assert len(by)==36

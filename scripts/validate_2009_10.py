...
data=json.loads((ROOT/'data/dashboard-2009-10.json').read_text(encoding='utf-8'))
assert len(data['tables'])==76
books={};count=0
...
assert 'Purchase' in by['5-2']['headers'][9]['measure']
assert 'Purchase' not in by['5-2']['headers'][10]['measure']
assert 'Australian sources' not in by['4-3']['headers'][4]['label']
assert 'Australian suppliers' not in by['4-5']['headers'][3]['label']
assert len({h['population'] for h in by['7-1']['headers']})==3
assert by['7-3']['headers'][8]['unit']=='$b'
assert len(by['7-3']['rows'])==5
assert len([r for r in by['9-1']['rows'] if r['group']=='Industry'])==17
assert all(t['scope']==('including' if int(t['id'].split('-')[0])%2 else 'excluding') for t in data['tables'])
assert all('Coverage: '+t['scope'] in h['denominator'] for t in data['tables'] for h in t['headers'])
assert not any(r['group']=='Industry' for t in data['tables'] if t['scope']=='excluding' for r in t['rows'])
assert all(r['group']=='Overall' for r in by['7-2']['rows'] if r['label']=='Total All Industries')
flags=Counter(c['status'] for t in data['tables'] for r in t['rows'] for c in r['cells'])
print(f'PASS: {count:,} observations match Excel; source coverage, population pivots, coverage sets, flags, units and header spans verified')
print(dict(flags))

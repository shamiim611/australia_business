"""Extract the 2011-12 publication with explicit header spans and population metadata."""
import json
import re
...
def extract():
    tables=[]
    for path in sorted((ROOT/'data_2011-12/extracted').glob('**/*')):
        ...
        for s in book.sheets()[1:]:
            ...
            for c in cols:
                ...
                if s.cell_value(u,c)=="'000":denominator='Estimated number of businesses at 30 June 2012, in thousands; contextual information only.'
    ...
    return {'period':'2011\u201312','year':'2011-12','released':'19 September 2013','tables':tables,'ui':{
...
if __name__=='__main__':
    ...
    for ext,content in [('json',encoded),('js','window.BUSINESS_DATA = '+encoded+';\n')]:
        (ROOT/'data'/f'dashboard-2011-12.{ext}').write_text(content,encoding='utf-8')

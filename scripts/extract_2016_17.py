"""Extract the 2016-17 publication with explicit header spans and population metadata."""
import json
import re
...
def extract():
    tables=[];seen_books={}
    for path in sorted((ROOT/'data_2016-17/extracted').glob('**/*')):
        ...
        workbook=topic
        topic={1:1,2:2,3:3,4:4,5:6,6:7,7:8}[workbook]
        for s in book.sheets()[1:]:
            ...
            for c in cols:
                ...
                if s.cell_value(u,c)=="'000":denominator='Estimated number of businesses at 30 June 2017, in thousands; contextual information only.'
    ...
    return {'period':'2016\u201317','year':'2016-17','released':'16 August 2018','tables':tables,'ui':{
        'topics':[['Technology',4],['Markets & procurement',2],['Finance',3],['Ownership & work',1],['Performance',6],['Barriers',7],['Skills',8]],
        'initialTopic':4,
        'cards':[['Internet access','4-1',0],['Web presence','4-1',1],['Social media presence','4-1',2],['Received orders online','4-1',4]],
        'overview':[['4-1',0,'Internet access'],['4-1',1,'Web presence'],['4-1',2,'Social media presence'],['4-1',3,'Placed orders online'],['4-1',4,'Received orders online']],
...
if __name__=='__main__':
    ...
    for ext,content in [('json',encoded),('js','window.BUSINESS_DATA = '+encoded+';\n')]:
        (ROOT/'data'/f'dashboard-2016-17.{ext}').write_text(content,encoding='utf-8')

...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2015-16.html');page.wait_for_url('**/2015-16.html');page.wait_for_selector('.bar-row')
    ...
    expected=page.evaluate("['4-1','4-1','4-1','5-1'].map((id,i)=>{const t=window.BUSINESS_DATA.tables.find(t=>t.id===id);return t.rows.find(r=>r.group==='Overall').cells[[0,1,4,2][i]].value.toFixed(1)+'%';})")
    ...
    page.locator('[data-topic="4"]').click();page.select_option('#metric','2')
    ...
    assert '2015-16' in dl.value.suggested_filename
    rows=list(csv.DictReader(io.StringIO(Path(dl.value.path()).read_text(encoding='utf-8-sig'))))
    assert rows and all(r['Period']=='2015\u201316' for r in rows)
    assert all('equity finance' in r['Notes'] for r in rows)
    page.locator('[data-topic="4"]').click();page.select_option('#table','4-3');page.select_option('#metric','2')
    assert 'billions' in page.locator('#chartTitle').inner_text()
    assert 'billion AUD' in page.locator('#chart').inner_text()
    page.select_option('#group','Employment size')
    page.select_option('#mode','populations')
    assert page.locator('.bar-row').count()==12, 'Three populations across four employment sizes'
    page.select_option('#mode','groups')
    page.locator('[data-topic="8"]').click();page.select_option('#table','8-3');page.select_option('#mode','populations')
    assert page.locator('.bar-row').count()==51, 'Three skill populations across 17 industries'
    page.locator('[data-topic="5"]').click()
    page.select_option('#table','5-9')
    assert page.locator('#population').input_value()=='Innovation-active businesses'
    assert 'Innovation-active businesses' in page.locator('#denominator').inner_text()
    assert page.locator('.bar-row').count()==17
    page.select_option('#table','5-1')
    (ROOT/'artifacts').mkdir(exist_ok=True)
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2015-16-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2015-16-mobile.png'),full_page=True)
    ...
    browser.close()
    print('PASS: all 47 tables, populations and four modes; denominators, CSV, year navigation, overview, mobile width and no JS errors')

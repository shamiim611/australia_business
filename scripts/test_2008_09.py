...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2008-09.html');page.wait_for_url('**/2008-09.html');page.wait_for_selector('.bar-row')
    ...
    assert '2008-09' in dl.value.suggested_filename
    rows=list(csv.DictReader(io.StringIO(Path(dl.value.path()).read_text(encoding='utf-8-sig'))))
    assert rows and all(r['Period']=='2008\u201309' for r in rows)
    ...
    assert page.locator('.bar-row').count()==48, 'Three skill populations across 16 industries'
    page.locator('[data-topic="5"]').click()
    (ROOT/'artifacts').mkdir(exist_ok=True)
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2008-09-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2008-09-mobile.png'),full_page=True)
    ...
    browser.close()
    print('PASS: all 44 tables, populations and four modes; denominators, CSV, year navigation, overview, mobile width and no JS errors')

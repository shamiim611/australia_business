...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2014-15.html');page.wait_for_url('**/2014-15.html');page.wait_for_selector('.bar-row')
    ...
    expected=page.evaluate("['7-1','7-1','7-1','7-1'].map((id,i)=>{const t=window.BUSINESS_DATA.tables.find(t=>t.id===id);return t.rows.find(r=>r.group==='Overall').cells[[10,11,12,14][i]].value.toFixed(1)+'%';})")
    ...
    assert '2014-15' in dl.value.suggested_filename
    rows=list(csv.DictReader(io.StringIO(Path(dl.value.path()).read_text(encoding='utf-8-sig'))))
    assert rows and all(r['Period']=='2014\u201315' for r in rows)
    ...
    page.select_option('#mode','groups')
    page.locator('[data-topic="8"]').click();page.select_option('#table','6-3');page.select_option('#mode','populations')
    assert page.locator('.bar-row').count()==51, 'Three skill populations across 17 industries'
    page.locator('[data-topic="4"]').click()
    (ROOT/'artifacts').mkdir(exist_ok=True)
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2014-15-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2014-15-mobile.png'),full_page=True)
    ...
    browser.close()
    print('PASS: all 35 tables, populations and four modes; denominators, CSV, year navigation, overview, mobile width and no JS errors')

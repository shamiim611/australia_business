"""Extract the 2010-11 publication with explicit header spans and population metadata."""
import json
import re
...
def extract():
    tables=[]
    for path in sorted((ROOT/'data_2010-11/extracted').glob('**/*')):
        ...
        for s in book.sheets()[1:]:
            ...
            for c in cols:
                ...
                if s.cell_value(u,c)=="'000":denominator='Estimated number of businesses at 30 June 2011, in thousands; contextual information only.'
    ...
    return {'period':'2010\u201311','year':'2010-11','released':'13 September 2012','tables':tables,'ui':{
        'topics':[['Innovation',5],['Technology',4],['Skills',8],['Markets & suppliers',2],['Finance',3],['Ownership, work & IP',1],['Performance',6],['Barriers',7]],
        'initialTopic':5,
        'cards':[['Internet access','4-1',8],['Web presence','4-1',9],['Received orders online','4-1',11],['Introduced goods or services innovation','5-1',2]],
...
if __name__=='__main__':
    ...
    for ext,content in [('json',encoded),('js','window.BUSINESS_DATA = '+encoded+';\n')]:
        (ROOT/'data'/f'dashboard-2010-11.{ext}').write_text(content,encoding='utf-8')

...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2024-25.html');page.wait_for_url('**/2024-25.html');page.wait_for_selector('.bar-row')
    assert '?' not in page.locator('body').inner_text()
    assert page.locator('#topics button').count()==12
    assert page.locator('#heatmap tbody tr').count()==17
    assert page.locator('#overviewGroup option').count()==2
    expected=page.evaluate("['4-1','4-3','4-4','6-2'].map((id,i)=>{const t=window.BUSINESS_DATA.tables.find(t=>t.id===id);return t.rows.find(r=>r.group==='Overall').cells[[9,0,4,0][i]].value.toFixed(1)+'%';})")
    ...
    for topic in [4,6,5,7,8,9,10,11,1,2,3,12]:
    ...
    page.locator('[data-topic="2"]').click();page.select_option('#table','2-3');page.select_option('#metric','0')
    assert 'sought debt or equity' in page.locator('#denominator').inner_text()
    ...
    assert rows and all(r['Period']=='2024\u201325' for r in rows)
    ...
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2024-25-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2024-25-mobile.png'),full_page=True)
    ...
    browser.close()
    print('PASS: all 61 tables, populations and four modes; denominators, CSV, year navigation, overview, mobile width and no JS errors')

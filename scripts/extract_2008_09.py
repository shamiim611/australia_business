"""Extract the 2008-09 publication with explicit header spans and population metadata."""
import json
import re
...
def extract():
    tables=[]
    for path in sorted((ROOT/'data_2008-09/extracted').glob('**/*')):
        ...
        for s in book.sheets()[1:]:
            ...
            for c in cols:
                ...
                if s.cell_value(u,c)=="'000":denominator='Estimated number of businesses at 30 June 2009, in thousands; contextual information only.'
    ...
    return {'period':'2008\u201309','year':'2008-09','released':'16 September 2010','tables':tables,'ui':{
...
if __name__=='__main__':
    ...
    for ext,content in [('json',encoded),('js','window.BUSINESS_DATA = '+encoded+';\n')]:
        (ROOT/'data'/f'dashboard-2008-09.{ext}').write_text(content,encoding='utf-8')

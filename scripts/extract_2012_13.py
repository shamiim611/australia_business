"""Extract the 2012-13 publication with explicit header spans and population metadata."""
import json
import re
...
def extract():
    tables=[]
    for path in sorted((ROOT/'data_2012-13/extracted').glob('**/*')):
        ...
        for s in book.sheets()[1:]:
            ...
            for c in cols:
                ...
                if s.cell_value(u,c)=="'000":denominator='Estimated number of businesses at 30 June 2013, in thousands; contextual information only.'
    ...
    return {'period':'2012\u201313','year':'2012-13','released':'18 September 2014','tables':tables,'ui':{
...
if __name__=='__main__':
    ...
    for ext,content in [('json',encoded),('js','window.BUSINESS_DATA = '+encoded+';\n')]:
        (ROOT/'data'/f'dashboard-2012-13.{ext}').write_text(content,encoding='utf-8')

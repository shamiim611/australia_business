...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2019-20.html');page.wait_for_url('**/2019-20.html');page.wait_for_selector('.bar-row')
    ...
    expected=page.evaluate("['1-1','1-1','1-1','4-1'].map((id,i)=>{const t=window.BUSINESS_DATA.tables.find(t=>t.id===id);return t.rows.find(r=>r.group==='Overall').cells[[0,1,2,1][i]].value.toFixed(1)+'%';})")
    ...
    page.locator('[data-topic="3"]').click();page.select_option('#table','3-2');page.select_option('#metric','2')
    ...
    assert rows and all(r['Period']=='2019\u201320' for r in rows)
    page.locator('[data-topic="4"]').click()
    (ROOT/'artifacts').mkdir(exist_ok=True)
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2019-20-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2019-20-mobile.png'),full_page=True)
    ...
    browser.close()
    print('PASS: all 22 tables, populations and four modes; denominators, CSV, year navigation, overview, mobile width and no JS errors')

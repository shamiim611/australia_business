...
with sync_playwright() as p:
    ...
    page.select_option('#yearNav','2006-07.html');page.wait_for_url('**/2006-07.html');page.wait_for_selector('.bar-row')
    assert '?' not in page.locator('body').inner_text()
    assert page.locator('#topics button').count()==7
    assert page.locator('#heatmap tbody tr').count()==16
    assert page.locator('#overviewGroup option').count()==2
    expected=page.evaluate("['4-1','4-1','4-1','5-1'].map((id,i)=>{const t=window.BUSINESS_DATA.tables.find(t=>t.id===id);return t.rows.find(r=>r.group==='Overall').cells[[8,9,11,2][i]].value.toFixed(1)+'%';})")
    ...
    assert '2006-07' in dl.value.suggested_filename
    rows=list(csv.DictReader(io.StringIO(Path(dl.value.path()).read_text(encoding='utf-8-sig'))))
    assert rows and all(r['Period']=='2006\u201307' for r in rows)
    assert all('equity finance' in r['Notes'] for r in rows)
    page.locator('[data-topic="4"]').click();page.select_option('#table','4-2');page.select_option('#mode','populations')
    assert page.locator('.bar-row').count()==15,'All three populations should match five size categories'
    page.select_option('#table','4-1')
    (ROOT/'artifacts').mkdir(exist_ok=True)
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2006-07-desktop.png'),full_page=True)
    page.set_viewport_size({'width':390,'height':844})
    assert page.evaluate('document.documentElement.scrollWidth<=innerWidth')
    page.screenshot(path=str(ROOT/'artifacts/dashboard-2006-07-mobile.png'),full_page=True)
    ...
    browser.close()
    print('PASS: all 33 tables, populations and four modes; denominators, CSV, year navigation, overview, mobile width and no JS errors')

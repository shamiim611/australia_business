"""Extract the 2014-15 publication with explicit header spans and population metadata."""
import json
import re
...
def extract():
    tables=[]
    for path in sorted((ROOT/'data_2014-15/extracted').glob('**/*')):
        if path.suffix.lower()!='.xls': continue
        book=xlrd.open_workbook(path,formatting_info=True)
        workbook=int(path.stem[7:10]);topic={1:1,2:2,3:3,4:6,5:7,6:8,7:4}[workbook]
        for s in book.sheets()[1:]:
            number=int(re.search(r'\d+$',s.name).group());actual_id=f'{workbook}-{number}'
            canonical=4 if topic==2 and number==3 else number
            ...
            for c in cols:
                ...
                for r in range(4,u):
                    ...
                    if topic==7 and r==4:
                        ...
                        local=(c-1)%width+1
                        if local not in [4,5,6]:continue
                ...
                if s.cell_value(u,c)=="'000":denominator='Estimated number of businesses at 30 June 2015, in thousands; contextual information only.'
    ...
    return {'period':'2014\u201315','year':'2014-15','released':'18 August 2016','tables':tables,'ui':{
        'topics':[['Technology',4],['Markets & procurement',2],['Finance',3],['Ownership, work & IP',1],['Performance',6],['Barriers',7],['Skills',8]],
        'initialTopic':4,
        'cards':[['Internet access','7-1',10],['Web presence','7-1',11],['Social media presence','7-1',12],['Received orders online','7-1',14]],
        'overview':[['7-1',10,'Internet access'],['7-1',11,'Web presence'],['7-1',12,'Social media presence'],['7-1',13,'Placed orders online'],['7-1',14,'Received orders online']],
...
if __name__=='__main__':
    ...
    for ext,content in [('json',encoded),('js','window.BUSINESS_DATA = '+encoded+';\n')]:
        (ROOT/'data'/f'dashboard-2014-15.{ext}').write_text(content,encoding='utf-8')
